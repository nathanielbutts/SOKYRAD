...
def create_map_and_grid(beamsize_fwhm, xcoords, ycoords, signal):
...
def plot_one(plot_type, projection, cygrid_map, levels, linestyle, chan, cmap):
    if plot_type == "contour":
        #do plot contour
        fig = plt.figure(figsize=(14, 11))
        ax1 = fig.add_subplot(111, projection=projection, aspect='auto') #change 111 to 221 if using 4 plots
        imkw = dict(origin='lower', interpolation='spline36') # spline36
        im1 = ax1.imshow(cygrid_map, **imkw, cmap=cmap) 
        cs1 = ax1.contour(
            cygrid_map, colors='black', linestyle=linestyle,
    ...
    if plot_type == "contourf":
        ...
        lon.set_axislabel('R.A. [deg]')
        lat.set_axislabel('Dec [deg]')
    # if plot_type == "contourcombine":
    #     # do plot contourcombine
    
    return plt

def plot_many(plot_type, projection, cygrid_map, levels, linestyle, cmap, plot_title):
    fig = plt.figure(figsize=(14, 11))
    ax1 = fig.add_subplot(221, projection=projection, aspect='auto') #change 111 to 221 if using 4 plots
    ax2 = fig.add_subplot(222, projection=projection, aspect='auto')
    ax3 = fig.add_subplot(223, projection=projection, aspect='auto')
    ax4 = fig.add_subplot(224, projection=projection, aspect='auto')

    imkw = dict(origin='lower', interpolation='spline36') # spline36
    im1 = ax1.imshow(cygrid_map, **imkw, cmap='jet') 
    im2 = ax2.imshow(cygrid_map, **imkw, cmap='jet') 
    im3 = ax3.imshow(cygrid_map, **imkw, cmap='jet') 
    im4 = ax4.imshow(cygrid_map, **imkw, cmap='jet') 

    fig.suptitle(plot_title, y=0.95)  # Adjust the y position
...
def main():
    ...
    linestyle = "dashed"
    cmap = "jet"
    plot_title = "Barnard 33 - Greenbank 20m"

    xcoords, ycoords, signal = setup_data(df, chan) #plot three 
    target_wcs, cygrid_map = create_map_and_grid(beamsize_fwhm, xcoords, ycoords, signal)
    
    plot = plot_one(plot_type, target_wcs.celestial, cygrid_map, levels, linestyle, chan, cmap)
    #plot = plot_many(plot_type, target_wcs.celestial, cygrid_map, levels, linestyle, cmap, plot_title)
